...
export function parseCSV(content: string): CSVAgency[] {
  try {
    ...
    const requiredHeaders = ['name', 'location', 'description', 'contact_email'];
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      try {
        ...
        if (values.length !== headers.length) {
          throw new Error(`Row ${i + 1} has ${values.length} columns but should have ${headers.length}`);
        }
        ...
        headers.forEach((header, colIndex) => {
          ...
          if (requiredHeaders.includes(header) && !value) {
            throw new Error(`Missing required value for ${header} in row ${i + 1}`);
          }
          ...
          switch (header) {
            case 'trust_score':
              ...
              if (value && (isNaN(score) || score < 0 || score > 100)) {
                throw new Error(`Trust score must be between 0 and 100 in row ${i + 1}`);
              }
            ...
            case 'price':
              ...
              if (value && (isNaN(price) || price < 0)) {
                throw new Error(`Price must be a positive number in row ${i + 1}`);
              }
